...
public class RegtreeFwd {
    ...
    private static int cut_instance_leaf_split_helper(Regtree tree, int thetacols, int thisnode) {
        int ret = 0, left_kid = tree.children[thisnode][0], right_kid = tree.children[thisnode][1];
        if (left_kid == 0) {
            ret = 1;
        } else if (cut_instance_leaf_split_helper(tree, thetacols, left_kid) 
                    +
                   cut_instance_leaf_split_helper(tree, thetacols, right_kid) 
                    == 2 
                   && Math.abs(tree.var[thisnode]) > thetacols
                ) {
                // both children are leaves, and this is a split on an instance
                tree.var[thisnode] = 0;
                make_leaf(tree, thisnode, left_kid, right_kid);
                ret = 1;
        }
        ...
        if (tree.weightedpred[thisnode] == 0) {
            tree.var[thisnode] = 0;
            make_leaf(tree, thisnode, left_kid, right_kid);
            ret = 1;
        }
    ...
    private static void make_leaf(Regtree tree, int thisnode, int left_kid, int right_kid) {
